...
const getDashboardSummary = async (req, res) => {
  ...
  try {
    ...
    // api calls used (total tokens used)
    const apiCallsUsed = await TokenUsageService.getTotalTokensUsed(userId);
    ...
    const data = {
      ...
      utilization: {
        ...
        tokens: { // Renamed from apiCalls to tokens for clarity based on screenshot
          used: apiCallsUsed,
          limit: apiCallsLimit,
...
const getTokenUsage = async (req, res) => {
  ...
  try {
    const totalUsed = await TokenUsageService.getTotalTokensUsed(userId);
    const remainingTokens = await TokenUsageService.getRemainingTokens(userId);
    ...
    console.error('getTokenUsage error:', err);
    return res.status(500).json({ success: false, message: 'Internal server error' });
  }
};
